        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 shrink-0">
              <h2 className="text-xl font-bold text-slate-800 tracking-tight">Konfirmasi Data Anggota</h2>
              <p className="text-sm text-slate-500 mt-1">Harap periksa kembali data di bawah ini sebelum menyimpan.</p>
            </div>
            
            <div className="p-6 overflow-y-auto space-y-6">
              <div>
                <h3 className="font-semibold text-slate-800 mb-3 border-b border-slate-100 pb-2">Data Anggota Utama</h3>
              ...
              {addSpouse && (
                <div>
                  <h3 className="font-semibold text-slate-800 mb-3 border-b border-slate-100 pb-2">Data Pasangan</h3>
                  <div className="grid grid-cols-2 gap-y-3 text-sm">
                    <div className="text-slate-500">Nama Lengkap:</div>
                    <div className="font-medium text-slate-800">{spouseData.full_name || '-'}</div>
                    <div className="text-slate-500">Jenis Kelamin:</div>
                    <div className="font-medium text-slate-800">{spouseData.gender === 'male' ? 'Laki-Laki' : 'Perempuan'}</div>
                    <div className="text-slate-500">Tanggal Lahir:</div>
                    <div className="font-medium text-slate-800">{spouseData.birth_date || '-'}</div>
                    <div className="text-slate-500">Tanggal Wafat:</div>
                    <div className="font-medium text-slate-800">{spouseData.death_date || '-'}</div>
                  </div>
                </div>
              )}
              ...
              {additionalChildren.length > 0 && (
                <div>
                  <h3 className="font-semibold text-slate-800 mb-3 border-b border-slate-100 pb-2">Data Anak Tambahan</h3>
                  <p className="text-sm text-slate-600 mb-3">{additionalChildren.length} anak tambahan akan ditambahkan bersamaan.</p>
                </div>
              )}
            </div>
